...
$limit=0;
		...
		while ($rid<=$last_rid)
		{
if (($limit++)>10) {die("/[loop!]");/*with summable 10 is enough?*/}
...
/*mis-using the class field for style and title is not the finest,..**/
			...
			$rows[count($rows)]=array(
				'data'=>array($this->rgroup[$rid][0],($this->summable?0:$running_sum))/*initialize sum with 0 (if summable) else take actual value*/
				,'translatable'=>false
				,'depth'=>$last_depth
...
print_r($this->rgroup);
print_r($rows);
die("</pre>appendStructureRange(&$rows,&$running_sum,$last_rid,".(count($this->rgroup)-1).",&$last_depth)");
*/
	        return array(
			'tables'=>array(/*array of tables*/
                                'main'=>array( /*table 1*/
					'type'=>'both',name=>'Unique Users'/*!!?? move to chartOptions?*/
					,'chartOptions'=>array('type'=>'ColumnChart','width'=>370,'height'=>300,'depths'=>array(1,2),'nativeOptions'=>"legend:{position :'none'}")
